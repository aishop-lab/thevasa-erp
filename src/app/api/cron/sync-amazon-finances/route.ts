...
export async function GET(request: NextRequest) {
  try {
    ...
    for (const platform of amazonPlatforms) {
      try {
        console.log(`[Cron Finances] Starting finance sync for team ${platform.team_id}`);
        const summary = await syncAmazonFinances(platform.team_id);
        results.push({ team_id: platform.team_id, summary });
        console.log(`[Cron Finances] Completed for team ${platform.team_id}`);
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        console.error(`[Cron Finances] Error for team ${platform.team_id}:`, msg);
    ...
    console.error('[Cron Finances] Fatal error:', error);
    return NextResponse.json(
