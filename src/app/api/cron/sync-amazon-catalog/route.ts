...
export async function GET(request: NextRequest) {
  try {
    ...
    for (const platform of amazonPlatforms) {
      try {
        console.log(`[Cron Catalog] Starting catalog sync for team ${platform.team_id}`);
        const summary = await syncAmazonCatalog(platform.team_id, 200);
        results.push({ team_id: platform.team_id, summary });
        console.log(`[Cron Catalog] Completed for team ${platform.team_id}`);
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        console.error(`[Cron Catalog] Error for team ${platform.team_id}:`, msg);
    ...
    console.error('[Cron Catalog] Fatal error:', error);
    return NextResponse.json(
